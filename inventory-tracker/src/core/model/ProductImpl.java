 ...
 * {@link Product} interface. The constructor(s) are (mostly) hidden. To get a
 * Product instance use the {@link Product.Factory}.
 * 
 * @author kemcqueen
 */
class ProductImpl extends AbstractContainable<ProductContainer> implements
		Product {
	...
	@Override
	public void wasAddedTo(final ProductContainer container)
			throws HITException {
	...
	@Override
	public void wasRemovedFrom(final ProductContainer container)
			throws HITException {
	...
	public void transfer(ProductContainer from, ProductContainer to)
		...
		// transfer all of the items associated with this product to the new
		// container
	...
	public Date getCreationDate() {
		...
		for (Item item : InventoryManager.Factory.getInventoryManager()
				.getItems(this)) {
			if(item.getEntryDate().before(earliestDate)){
				earliestDate = item.getEntryDate();
			}
		}
		setCreationDate(earliestDate);
	...
	public Iterable<StorageUnit> getStorageUnits() {
		assert true;
		return Collections.unmodifiableSet(this.containersByStorageUnit
				.keySet());
	}
	...
	public Iterable<ProductContainer> getProductContainers() {
		assert true;
		return Collections.unmodifiableCollection(this.containersByStorageUnit
				.values());
	}
	...
	@Override
	protected void verifyContainedIn(ProductContainer container)
			throws HITException {
		if (null == container) {
		...
		if (false == container.containsProduct(this)) {
			throw new HITException(HITException.Severity.WARNING, "Container ("
					+ container + ") does not contain this product");
		}
	}

	@Override
	protected void verifyNotContainedIn(ProductContainer container)
			throws HITException {
		if (null == container) {
			throw new HITException(HITException.Severity.WARNING,
					"Container must not be null");
		...
		if (container.containsProduct(this)) {
			throw new HITException(HITException.Severity.WARNING, "Container ("
					+ container + ") still contains this product");
		}
